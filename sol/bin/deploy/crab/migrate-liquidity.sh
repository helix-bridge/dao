...
data1=$(seth calldata "removeLiquidity(address,address,uint,uint,uint,address,uint)" $USDT $xRING $USDT_xRING_LIQ $USDT_AMOUT_OUT $xRING_AMOUT_OUT0 $TIMELOCK $DEADLINE)
...
data2=$(seth calldata "removeLiquidity(address,address,uint,uint,uint,address,uint)" $xRING $WCRAB $WCRAB_xRING_PAIR $xRING_AMOUT_OUT1 $WCRAB_AMOUT_OUT $TIMELOCK $DEADLINE)

MigrateBL=$(cal "$xRING_BL0 + $xRING_AMOUT_OUT0 + $xRING_AMOUT_OUT1")

data3=$(seth calldata "approve(address,uint)" $xRING_MIGRATOR $MigrateBL)
data4=$(seth calldata "migrateAll()")

data5=$(seth calldata "approve(address,uint)" $ROUTER $USDT_AMOUT_OUT)
data6=$(seth calldata "approve(address,uint)" $ROUTER $(($xRING_AMOUT_OUT0 + $xRING_AMOUT_OUT1)))
data7=$(seth calldata "approve(address,uint)" $ROUTER $WCRAB_AMOUT_OUT)

USDT_MIN=$(cal "$USDT_AMOUT_OUT * 0.99")
xWRING_MIN0=$(cal "$xRING_AMOUT_OUT0 * 0.99")
data8=$(seth calldata "addLiquidity(address,address,uint,uint,uint,uint,address,uint)" $USDT $xWRING $USDT_AMOUT_OUT $xRING_AMOUT_OUT0 $USDT_MIN $xWRING_MIN0 $TIMELOCK $DEADLINE)

WCRAB_MIN=$(cal "$WCRAB_AMOUT_OUT *0.99")
xWRING_MIN1=$(cal "$xRING_AMOUT_OUT1 *0.99")
data9=$(seth calldata "addLiquidity(address,address,uint,uint,uint,uint,address,uint)" $WCRAB $xWRING $WCRAB_AMOUT_OUT $xRING_AMOUT_OUT1 $WCRAB_MIN $xWRING_MIN1 $TIMELOCK $DEADLINE)

targets=[$ROUTER,$ROUTER,$xRING,$xRING_MIGRATOR,$USDT,$xWRING,$WCRAB,$ROUTER,$ROUTER]
